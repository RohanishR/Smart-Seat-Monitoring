  ...
  if (align_face(net_boxes, image_matrix, aligned_face) == ESP_OK) {
    if (is_enrolling == 1) {
      ...
      if (matched_id >= 0) {
        ...
        rgb_printf(image_matrix, FACE_COLOR_GREEN, "Hello %s", students[matched_id]);

        Serial.print("Attendance Marked: ");
        Serial.println(students[matched_id]);

        if (!marked[matched_id] && attendanceIndex < 20) {
          attendanceList[attendanceIndex] = students[matched_id];
          attendanceIndex++;
        ...
        rgb_print(image_matrix, FACE_COLOR_RED, "No Match Found!");
        matched_id = -1;
      }
    }
  } else {
...
static esp_err_t attendance_handler(httpd_req_t *req) {
  // Create a JSON array instead of an HTML table
  String json = "[";
  for (int i = 0; i < attendanceIndex; i++) {
    json += "{\"id\":" + String(i + 1) + ", \"name\":\"" + attendanceList[i] + "\"}";
    if (i < attendanceIndex - 1) {
      json += ",";
    }
  }
  json += "]";
  ...
  httpd_resp_set_type(req, "application/json");
  // CRITICAL: Allows your external dashboard.html to fetch this data
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*"); 
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &attendance_uri);
  }
